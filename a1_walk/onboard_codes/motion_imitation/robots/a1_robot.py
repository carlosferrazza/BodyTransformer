...
#     c = q_vec * \
#         q_vec.reshape((1, 3)) @ v.reshape((3, 1)) * 2.0
#     return a - b + c

def quat_rotate_inverse(q, v):
    
    q = torch.tensor(q).unsqueeze(0).float()
...
class A1Robot(a1.A1):
  ...
  def __init__(self,
    ...
    print("Action filter enabled: ", self._enable_action_filter)
  ...
  def GetBaseQuaternion(self):
    return np.array(self._raw_state.imu.quaternion).copy()
  ...
  def GetProjectedGravity(self):
    ...
    return quat_rotate_inverse(self._base_orientation, gravity)
