...
LIMITS = {
    'hip_max': 0.802851455917,
    'hip_min': -0.802851455917,
    'thigh_max': 4.18879020479,
    'thigh_min': -1.0471975512,
    'calf_max': -0.916297857297,
    'calf_min': -2.69653369433
}
...
class UnitreeA1Real:
    ...
    def start_noros(self):
        ...
        self.robot = a1_robot.A1Robot(pybullet_client=p, action_repeat=self.cfg["control"]["decimation"], enable_action_filter=True, enable_action_interpolation=True, time_step=self.cfg["sim"]["dt"])
        # self.robot = a1_robot.A1Robot(pybullet_client=p, action_repeat=1, enable_action_filter=False, enable_action_interpolation=True)
        self.robot.HoldCurrentPose()
    ...
    def process_configs(self):
        ...
        print("Computer Clip Torque (onboard) is " + str(self.computer_clip_torque))
        if self.computer_clip_torque:
            self.torque_limits = self.extra_cfg["torque_limits"]
            print("[Env] torque limit: {:.1f} {:.1f} {:.1f}".format(*self.torque_limits[:3]))
        ...
        print("[Env] action scale: {:.1f}".format(self.action_scale))
        self.clip_actions = self.cfg["normalization"]["clip_actions"]
        if self.cfg["normalization"].get("clip_actions_method", None) == "hard":
            print("clip_actions_method with hard mode")
            print("clip_actions_high: " + str(self.cfg["normalization"]["clip_actions_high"]))
            print("clip_actions_low: " + str(self.cfg["normalization"]["clip_actions_low"]))
    ...
    def _get_proprioception_obs(self):\
        ...
        base_ang_vel = torch.tensor(self.robot.GetBaseRollPitchYawRate(), device= self.model_device).unsqueeze(0)
        projected_gravity = quat_rotate_inverse(
            torch.tensor(self.robot.GetBaseOrientation(), dtype=torch.float32).unsqueeze(0),
            self.gravity_vec,
        ).to(self.model_device)
        dof_pos = self.robot.GetTrueMotorAngles()
        self.dof_pos = dof_pos = torch.tensor([dof_pos[self.dof_map[i]] for i in range(12)], dtype= torch.float32, device= self.model_device).unsqueeze(0)
        dof_vel = self.robot.GetMotorVelocities()
        self.dof_vel = dof_vel = torch.tensor([dof_vel[self.dof_map[i]] for i in range(12)], dtype= torch.float32, device= self.model_device).unsqueeze(0)
        ...
        # print("base_ang_vel: ", base_ang_vel.device)
        # print("projected_gravity: ", projected_gravity.device)
        # print("dof_pos: ", dof_pos.device)
        # print("dof_vel: ", dof_vel.device)
        # print("command_buf: ", self.command_buf.device)

        self.command_buf[0,0] = 0.5

        # print("base_ang_vel: ", base_ang_vel)
        # print("projected_gravity: ", projected_gravity)
        # print("dof_pos: ", dof_pos)
        # print("dof_vel: ", dof_vel)
        # print("command_buf: ", self.command_buf)
        # print("actions: ", self.actions)

        # print("actions: ", self.actions.device)
        ...
        return torch.cat([
            ...
            dof_vel * self.obs_scales["dof_vel"] * scale_lin_vel,
            self.actions
        ], dim= -1).float()
    ...
    def _get_forward_depth_obs(self):
        if not self.forward_depth_embedding_dims:
            ...
            if self.low_state_get_time.to_sec() - self.forward_depth_embedding_stamp.to_sec() > 0.4:
                print("Getting depth embedding later than low_state later than 0.4s")
    ...
    def send_action(self, actions):
        ...
        """
        self.actions = self.clip_action_before_scale(actions).cpu()
        if self.computer_clip_torque:
            robot_coordinates_action = self.clip_by_torque_limit(actions.cpu() * self.action_scale) + self.default_dof_pos.unsqueeze(0)
        else:
            print("You are using control without any torque clip. The network might output torques larger than the system can provide.")
        ...
        self.publish_legs_cmd(robot_coordinates_action, 100, 1.0)
    ...
    def publish_legs_cmd(self, robot_coordinates_action, kp= None, kd= None):
        ...
        legs_cmd = np.zeros((12, 5))
        for sim_joint_idx in range(12):
            real_joint_idx = self.dof_map[sim_joint_idx]
            # legs_cmd.cmd[real_joint_idx].mode = 10
            legs_cmd[real_joint_idx, 0] = robot_coordinates_action[0, sim_joint_idx]
            legs_cmd[real_joint_idx, 2] = 0. # dq
            legs_cmd[real_joint_idx, 4] = 0. # tau
            legs_cmd[real_joint_idx, 1] = self.p_gains[sim_joint_idx] if kp is None else kp
            legs_cmd[real_joint_idx, 3] = self.d_gains[sim_joint_idx] if kd is None else kd
        
        self.robot.Step(legs_cmd.ravel(), robot_config.MotorControlMode.HYBRID)
    ...
    def update_low_state(self, ros_msg):
        ...
        if self.move_by_wireless_remote:
            ...
            if np.abs(self.command_buf[0, 2]) < self.ang_vel_deadband:
                self.command_buf[0, 2] = 0.
        self.low_state_get_time = time.time()
    ...
    def update_forward_depth_embedding(self, ros_msg):
        print("a1_ros_run recieved forward depth embedding.")
