...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        self.stack_size = params['network'].get('lookback_steps', 1)
    ...
    def get_action_values(self, obs):
        ...
        input_dict = {
            ...
            'mapping' : obs['mapping'] if 'mapping' in obs else None
        }
    ...
    def init_tensors(self):
        ...
        # print("env info:", self.env_info['observation_space'].shape)
        if self.model.is_stack():
            print("stacking observations")
            self.env_info['observation_space'] = gym.spaces.Box(np.tile(self.env_info['observation_space'].low, (self.stack_size,1)),
                                                                np.tile(self.env_info['observation_space'].high, (self.stack_size,1)),
                                                                dtype=self.env_info['observation_space'].dtype)
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            # print("obses:", self.obs['obs'].shape)
        ...
        batch_dict['mapping'] = self.obs['mapping'] if 'mapping' in self.obs else None

        return batch_dict

    def play_steps_stack(self):
        update_list = self.update_list

        step_time = 0.0

        stack = deque(maxlen=self.stack_size)
        for _ in range(self.stack_size):
            stack.append(torch.zeros_like(self.obs['obs']))

        for n in range(self.horizon_length):
            stack.append(self.obs['obs']) 
            obs = torch.stack(list(stack), dim=1)
            self.obs['obs'] = obs
            if self.use_action_masks:
                masks = self.vec_env.get_action_masks()
                res_dict = self.get_masked_action_values(self.obs, masks)
...
class ContinuousA2CBase(A2CBase):
    ...
    def train_epoch(self):
        ...
        self.set_eval()
        play_time_start = time.time()
        assert not (self.is_stack and self.is_rnn), "Stack and RNN are not supported together"
        if self.is_stack:
            batch_dict = self.play_steps_stack()
        elif self.is_rnn:
            batch_dict = self.play_steps_rnn()
        else:
            batch_dict = self.play_steps()
    ...
    def prepare_dataset(self, batch_dict):
        ...
        rnn_states = batch_dict.get('rnn_states', None)
        rnn_masks = batch_dict.get('rnn_masks', None)
        mappings = batch_dict.get('mapping', None)
