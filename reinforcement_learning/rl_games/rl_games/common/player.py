...
class BasePlayer(object):
    ...
    def __init__(self, params):
        ...
        print("Player stack size:", self.stack_size) 
        ...
        self.log_dir = self.player_config.get('log_dir', 'logs')
        ...
        self.log_data = self.player_config.get('log_data', False)
        print("Player log data:", self.log_data)
    ...
    def run(self):
        ...
        if self.log_data:
            ...
            self.log_obs = open(os.path.join(self.log_dir, "obs.txt"), "a")
            self.log_r = open(os.path.join(self.log_dir, "r.txt"), "a")
            self.log_action = open(os.path.join(self.log_dir, "action.txt"), "a")
            self.log_done = open(os.path.join(self.log_dir, "done.txt"), "a")
        ...
        for ng in range(n_games):
            ...
            for _ in range(stack_size):
                stack.append(torch.zeros_like(obses))
            ...
            for n in range(self.max_steps):
                ...
                if self.log_data:
                    # Write to the file
                    np.savetxt(self.log_obs, obses[:,-1,:].cpu().numpy())
                    self.log_obs.write("\n")
                    np.savetxt(self.log_action, action.cpu().numpy())
                    self.log_action.write("\n")

                obses, r, done, info = self.env_step(self.env, action)

                if self.log_data:
                    # Write to the file
                    np.savetxt(self.log_r, r.cpu().numpy())
                    self.log_r.write("\n")
                    np.savetxt(self.log_done, done.cpu().numpy())
                    self.log_done.write("\n")
                ...
                if done_count > 0:
                    ...
                    if self.print_stats:
                        ...
                        if print_game_res:
                            ...
                            print(f'games: {games_played} reward: {cur_rewards_done:.4} steps: {cur_steps_done:.4f}')
