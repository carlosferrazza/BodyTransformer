    ...
    def norm_obs(self, observation):
        with torch.no_grad():
            ...
            if len(observation.shape) == 3:
                masks = torch.zeros_like(observation)
                masks[:,2,:] = 1
            return self.running_mean_std(observation, mask=masks) if self.normalize_input else observation
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def is_stack(self):
            try:
                return self.a2c_network.is_stack()
            except:
                return False
