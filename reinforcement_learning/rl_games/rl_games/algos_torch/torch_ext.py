...
def get_mean_var_with_masks(values, masks, axis=None):
    if axis is None:
        sum_mask = masks.sum()
        ...
        values_mask = values[masks.bool()].reshape(values.shape[0], -1)
        values_mean = values_mask.mean(axis=axis)
        values_var = values_mask.var(axis=axis)
